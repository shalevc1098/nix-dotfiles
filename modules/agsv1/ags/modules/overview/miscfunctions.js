...
export function launchCustomCommand(command) {
    ...
    if (args[0] == '>raw') { // Mouse raw input
        ...
        if (!args[1])
            execAsync([`bash`, `-c`, `${App.configDir}/scripts/color_generation/switchwall.sh --color`, `&`]).catch(print);
        else if (args[1][0] === '#')
            execAsync([`bash`, `-c`, `${App.configDir}/scripts/color_generation/switchwall.sh --color "${args[1]}"`, `&`]).catch(print);
    }
