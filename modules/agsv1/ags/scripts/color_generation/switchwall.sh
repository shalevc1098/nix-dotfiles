...
handle_kde_material_you_colors() {
    ...
    case "$type_flag" in
        scheme-content|scheme-expressive|scheme-fidelity|scheme-fruit-salad|scheme-monochrome|scheme-neutral|scheme-rainbow|scheme-tonal-spot|scheme-vibrant)
            ...
            ;;
        *)
            kde_scheme_variant="scheme-vibrant" # default
            ;;
    esac
...
switch() {
    ...
    # Use command line args if provided, otherwise use colormode.txt values
    if [[ -z "$mode_flag" ]]; then
        mode_flag="$lightdark"
    fi
    
    # Use scheme from args if provided, otherwise from colormode.txt, and add scheme- prefix
    if [[ -z "$type_flag" ]]; then
        type_flag="$materialscheme"
    fi
    ...
    if [[ "$color_flag" == "1" ]]; then
        ...
        # Only prompt for upscaling if enabled and not using --noswitch
        if [[ "$should_prompt_upscale" == "true" && -z "$noswitch_flag" ]]; then
    ...
    if [[ -z "$mode_flag" ]]; then
        mode_flag="$lightdark"  # Use from colormode.txt instead of gsettings
    fi

    [[ -n "$mode_flag" ]] && matugen_args+=(--mode "$mode_flag") && generate_colors_material_args+=(--mode "$mode_flag")
    [[ -n "$type_flag" ]] && matugen_args+=(--type "$type_flag") && generate_colors_material_args+=(--scheme "$type_flag")
    
    # Check if scheme is monochrome and override terminalscheme
    if [[ "$type_flag" == "scheme-monochrome" ]]; then
    ...
    generate_colors_material_args+=(--transparency "$transparency")  # Add transparency from colormode.txt

    pre_process "$mode_flag"

    matugen "${matugen_args[@]}"
    # source "$(eval echo $ILLOGICAL_IMPULSE_VIRTUAL_ENV)/bin/activate"
    # echo "generate_colors_material_args: ${generate_colors_material_args[@]}"
...
main() {
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --noswitch)
                noswitch_flag="1"
                imgpath=$(swww query | head -n 1 | awk -F 'image: ' '{print $2}')
                shift
                ;;
            *)
                if [[ -z "$imgpath" ]]; then
