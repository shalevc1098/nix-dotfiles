...
apply_hyprlock() {
  ...
  wallpath=$(swww query | head -n 1 | awk -F 'image: ' '{print $2}')
  
  if [ -n "$wallpath" ]; then
    # Escape wallpaper path for sed (ensure \, &, and | are escaped for the replacement string)
    escaped_wallpath=$(echo "$wallpath" | sed -e 's/\\\\/\\\\\\\\/g' -e 's/&/\\\\&/g' -e 's/|/\\\\|/g')
    # Replace placeholder with actual wallpaper path, using | as delimiter
    sed -i "s|{{ SWWW_WAL }}|${escaped_wallpath}|g" "$CACHE_DIR"/user/generated/hypr/hyprlock.conf
  else
    echo "Warning: apply_hyprlock - Could not retrieve wallpaper path from swww. {{ SWWW_WAL }} will not be replaced."
  fi
